...
use chatgpt_plus_core::models::{DeleteResult, ExportResult, SessionRef};
use chatgpt_plus_core::routes::{BridgeContext, BridgeDataService, BridgeRuntimeService};
use chatgpt_plus_core::user_scripts::UserScriptManager;
...
fn should_recover_stale_launcher(debug_port: u16) -> bool {
    ...
    let cdp_listening = chatgpt_plus_core::watcher::cdp_listening(debug_port);
    let recover =
        chatgpt_plus_core::watcher::should_recover_stale_launcher(has_codex_process, cdp_listening);
    let _ = chatgpt_plus_core::diagnostic_log::append_diagnostic_log(
...
async fn activate_existing_codex_app(options: &LaunchOptions) -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        for process_id in &process_ids {
            if chatgpt_plus_core::windows_activate_process_window(*process_id) {
                activated = true;
                break;
            }
        }
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: chatgpt_plus_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    async fn delete(&self, session: SessionRef) -> anyhow::Result<DeleteResult> {
        ...
        let backup_store = chatgpt_plus_data::BackupStore::new(self.backup_dir.clone());
        tokio::task::spawn_blocking(move || {
            chatgpt_plus_data::delete_local_from_paths(db_paths, backup_store, &session)
        })
        .await
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let db_paths = self.candidate_db_paths();
        tokio::task::spawn_blocking(move || {
            chatgpt_plus_data::export_markdown_from_paths(db_paths, &session)
        })
        .await
    ...
    async fn move_thread_workspace(
        ...
        let backup_store = chatgpt_plus_data::BackupStore::new(self.backup_dir.clone());
        tokio::task::spawn_blocking(move || {
            chatgpt_plus_data::move_codex_thread_workspace_from_paths(
                db_paths,
                backup_store,
                &session,
                &target_cwd,
...
impl LauncherDataService {
    fn candidate_db_paths(&self) -> Vec<PathBuf> {
        ...
        for path in chatgpt_plus_core::codex_sqlite::codex_session_db_paths_from_home(
    ...
    fn storage_adapter(&self) -> chatgpt_plus_data::SQLiteStorageAdapter {
        chatgpt_plus_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            chatgpt_plus_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            chatgpt_plus_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let targets = chatgpt_plus_core::cdp::list_targets(debug_port).await?;
        let target = chatgpt_plus_core::cdp::pick_page_target(&targets)?;
        let url = chatgpt_plus_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn backend_status(&self) -> anyhow::Result<Value> {
        Ok(
            json!({"status": "ok", "message": "后端已连接", "version": chatgpt_plus_core::version::VERSION}),
        )
    }

    async fn codex_model_catalog(&self) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::model_catalog::read_codex_model_catalog().await)
    }

    async fn ads(&self) -> anyhow::Result<Value> {
        chatgpt_plus_core::ads::fetch_ad_list().await
    }

    async fn zed_remote_status(&self) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::zed_remote_status())
    }

    async fn resolve_zed_remote_host(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::resolve_ssh_target_response(
            &payload,
        ))
    }

    async fn fallback_zed_remote_request(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::fallback_open_request_response(&payload))
    }

    async fn open_zed_remote(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::open_zed_remote(&payload))
    }

    async fn list_zed_remote_projects(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::list_zed_remote_projects_response(&payload))
    }

    async fn remember_zed_remote_project(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::remember_zed_remote_project_response(&payload))
    }

    async fn forget_zed_remote_project(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::zed_remote::forget_zed_remote_project_response(&payload))
    }

    async fn upstream_worktree_status(&self) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::upstream_worktree::status_response())
    }

    async fn upstream_worktree_defaults(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::upstream_worktree::defaults_response(
            &payload,
        ))
    }

    async fn upstream_worktree_prepare(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::upstream_worktree::prepare_response(
            &payload,
        ))
    }

    async fn upstream_worktree_create(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(chatgpt_plus_core::upstream_worktree::create_response(
            &payload,
        ))
    }
}
...
async fn try_inject_with_context(
    ...
    let targets = chatgpt_plus_core::cdp::list_targets(debug_port).await?;
    let target = chatgpt_plus_core::cdp::pick_injectable_codex_page_target(&targets)?;
    let websocket_url = target
        .web_socket_debugger_url
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("selected CDP target has no websocket URL"))?;
    runtime.set_websocket_url(websocket_url);
    let settings = chatgpt_plus_core::settings::SettingsStore::default()
        .load()
        .unwrap_or_default();
    let script = chatgpt_plus_core::assets::injection_script_with_settings(helper_port, &settings);
    ...
    chatgpt_plus_core::bridge::install_bridge(
        websocket_url,
        chatgpt_plus_core::bridge::BRIDGE_BINDING_NAME,
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(chatgpt_plus_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
...
fn default_codex_db_path() -> PathBuf {
    chatgpt_plus_core::codex_sqlite::codex_session_db_path()
}
...
fn default_user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        if let Some(roaming) = std::env::var_os("APPDATA") {
            return chatgpt_plus_core::branding::resolve_branded_config_dir(&PathBuf::from(
                roaming,
            ));
        }
        if let Some(home) = directories::BaseDirs::new().map(|dirs| dirs.home_dir().to_path_buf()) {
            return chatgpt_plus_core::branding::resolve_branded_config_dir(
    ...
    chatgpt_plus_core::branding::resolve_branded_config_dir(&base)
}
...
mod tests {
    ...
    #[test]
    fn launcher_app_has_no_relay_profile_apply_hook_adapter() {
        let source = include_str!("main.rs");
        let removed_method = ["apply_active_", "relay_profile"].concat();

        assert!(!source.contains(&removed_method));
    }
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(chatgpt_plus_core::install::MANAGER_BINARY))
        );
    }
}
