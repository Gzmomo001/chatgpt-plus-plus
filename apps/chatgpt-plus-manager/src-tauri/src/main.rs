...
fn main() {
    ...
    chatgpt_plus_manager_lib::run();
}

#[cfg(windows)]
fn focus_existing_manager_window() {
    let current_process_id = std::process::id();
    for process in chatgpt_plus_core::windows_enumerate_processes() {
        if process.process_id == current_process_id {
            continue;
        }
        if process
            .exe_file
            .eq_ignore_ascii_case("chatgpt-plus-plus-manager.exe")
        {
            let _ = chatgpt_plus_core::windows_activate_process_window(process.process_id);
            break;
        }
    }
}
