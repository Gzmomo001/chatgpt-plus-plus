...
use chatgpt_plus_core::settings::{BackendSettings, RelayProfile, SettingsStore};
use chatgpt_plus_core::status::{LaunchStatus, StatusStore};
use chatgpt_plus_core::user_scripts::UserScriptManager;
use chatgpt_plus_core::zed_remote::{ZedOpenStrategy, ZedRemoteProject};
use serde::Serialize;
...
pub fn launch_chatgpt_plus(request: LaunchRequest) -> CommandResult<Value> {
    spawn_chatgpt_plus_launch(request, "启动任务已在后台开始，可稍后查看概览状态。")
}

#[tauri::command]
pub fn restart_chatgpt_plus(request: LaunchRequest) -> CommandResult<Value> {
    chatgpt_plus_core::watcher::stop_launcher_processes_and_wait();
    chatgpt_plus_core::watcher::stop_codex_processes_and_wait();
    spawn_chatgpt_plus_launch(request, "Codex 已请求重启，启动任务正在后台运行。")
}

fn spawn_chatgpt_plus_launch(
...
fn spawn_silent_launcher(request: &LaunchRequest) -> anyhow::Result<()> {
    let launcher = chatgpt_plus_core::install::companion_binary_path(SILENT_BINARY);
...
pub fn list_local_sessions() -> CommandResult<LocalSessionsPayload> {
    let home = chatgpt_plus_core::codex_sqlite::default_codex_home_dir();
...
pub fn list_zed_remote_projects() -> CommandResult<ZedRemoteProjectsPayload> {
    let result = chatgpt_plus_core::zed_remote::list_zed_remote_projects_response(&json!({}));
...
pub fn open_zed_remote(payload: Value) -> CommandResult<ZedRemoteOpenPayload> {
    let result = chatgpt_plus_core::zed_remote::open_zed_remote(&payload);
    let strategy = result
        .get("strategy")
        .cloned()
...
pub fn forget_zed_remote_project(id: String) -> CommandResult<ZedRemoteProjectsPayload> {
    let result =
        chatgpt_plus_core::zed_remote::forget_zed_remote_project_response(&json!({ "id": id }));
    if result.get("status").and_then(Value::as_str) != Some("ok") {
        return failed(
            result
                .get("message")
...
pub fn delete_local_session(request: DeleteLocalSessionRequest) -> CommandResult<DeleteResult> {
    ...
    if session_id.is_empty() {
        return failed(
            "会话 ID 不能为空。",
            DeleteResult {
                status: chatgpt_plus_core::models::DeleteStatus::Failed,
    ...
    let result = chatgpt_plus_data::delete_local_from_paths(
        candidate_paths.clone(),
        chatgpt_plus_data::BackupStore::new(
            chatgpt_plus_core::paths::default_app_state_dir().join("backups"),
        ),
        &session,
    );
    log_manager_event(
...
fn local_session_adapter(db_path: &Path) -> chatgpt_plus_data::SQLiteStorageAdapter {
...
fn normalize_settings_before_save(mut settings: BackendSettings) -> BackendSettings {
    if let Some(path) =
        chatgpt_plus_core::app_paths::normalize_codex_app_path(Path::new(&settings.codex_app_path))
    {
        settings.codex_app_path = path.to_string_lossy().to_string();
    }
    settings.relay_common_config_contents =
        chatgpt_plus_core::relay_config::sanitize_common_config_contents(
            &settings.relay_common_config_contents,
        );
    ...
    for profile in &mut settings.relay_profiles {
        if let Err(error) =
            chatgpt_plus_core::relay_config::normalize_relay_profile_for_storage(profile)
        {
            log_manager_event(
    ...
    if !common_config.trim().is_empty() {
        for profile in &mut settings.relay_profiles {
            ...
            match chatgpt_plus_core::relay_config::strip_common_config_from_config(
...
fn relay_join_config_sections(sections: &[&str]) -> String {
    ...
    if sections.is_empty() {
        ...
        chatgpt_plus_core::relay_config::normalize_config_text(&format!(
            "{}\n",
            sections.join("\n\n")
        ))
    }
...
pub async fn load_provider_sync_targets() -> CommandResult<Value> {
    ...
    let result = tauri::async_runtime::spawn_blocking(|| {
        chatgpt_plus_data::load_provider_sync_targets(None)
    })
    .await
...
fn merge_manual_provider_sync_targets(
    targets: &mut chatgpt_plus_data::ProviderSyncTargetList,
    manual: &[String],
    ...
    for id in manual {
        if let Some(existing) = targets.targets.iter_mut().find(|target| target.id == *id) {
            if !existing
                .sources
                .contains(&chatgpt_plus_data::ProviderSyncTargetSource::Manual)
            {
                existing
                    .sources
                    .push(chatgpt_plus_data::ProviderSyncTargetSource::Manual);
            ...
            targets
                .targets
                .push(chatgpt_plus_data::ProviderSyncTargetOption {
                    id: id.clone(),
                    sources: vec![chatgpt_plus_data::ProviderSyncTargetSource::Manual],
...
pub async fn sync_providers_now(target_provider: Option<String>) -> CommandResult<Value> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        chatgpt_plus_data::run_provider_sync_with_target(None, target_provider.as_deref())
    })
    .await
...
fn is_success_sync_status(status: &chatgpt_plus_data::ProviderSyncStatus) -> bool {
    matches!(status, chatgpt_plus_data::ProviderSyncStatus::Synced)
}
...
pub async fn diagnose_relay_profile(profile: RelayProfile) -> CommandResult<ProviderDoctorPayload> {
    ...
    if profile.relay_mode == chatgpt_plus_core::settings::RelayMode::Official
    ...
    if chatgpt_plus_core::relay_config::relay_profile_base_url(&profile)
        .trim()
        .is_empty()
        || chatgpt_plus_core::relay_config::relay_profile_api_key(&profile)
            .trim()
            .is_empty()
    {
        checks.push(ProviderDoctorCheck {
    ...
    checks.push(ProviderDoctorCheck {
        ...
        detail: format!(
            "{} / {}",
            chatgpt_plus_core::relay_config::relay_profile_base_url(&profile),
            match profile.protocol {
                chatgpt_plus_core::settings::RelayProtocol::Responses => "Responses API",
                chatgpt_plus_core::settings::RelayProtocol::ChatCompletions => "Chat Completions",
            }
        ),
    });
...
fn reconcile_relay_injection_in_home(
    event: &str,
    home: &Path,
    settings: &BackendSettings,
    success_message: &str,
...
fn clear_relay_injection_in_home(
    home: &Path,
    settings: &BackendSettings,
) -> CommandResult<RelayPayload> {
...
fn empty_context_entries() -> chatgpt_plus_core::relay_config::CodexContextEntries {
...
fn load_overview_payload() -> (
    ...
    let settings = SettingsStore::default().load().unwrap_or_default();
    (
        chatgpt_plus_core::app_paths::resolve_codex_app_dir_with_saved(
            None,
