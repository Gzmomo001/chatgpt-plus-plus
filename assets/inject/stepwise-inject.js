(() => {
  ...
  const runtimeSurface = window.__chatgptPlusRuntimeCompatibility?.dom?.classifySurface?.() || null;
  if (runtimeSurface?.kind === "auxiliary") {
  ...
  function toggleCodexTheme() {
    ...
    setCodexThemeMode(nextTheme)
      ...
      .catch((error) => {
        console.warn("[ChatGPT++ Stepwise] Failed to switch Codex theme", error);
      });
  }
