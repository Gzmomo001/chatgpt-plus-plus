...
DMG="$DIST/ChatGPTPlusPlus-${VERSION}-macos-${ARCH}.dmg"
ICON_SOURCE="$ROOT/apps/chatgpt-plus-manager/src-tauri/icons/icon.png"
ICON_NAME="chatgpt-plus-plus.icns"
...
prepare_icon() {
  local iconset="$DIST/chatgpt-plus-plus.iconset"
  rm -rf "$iconset"
  mkdir -p "$iconset"
...
hdiutil create -volname "ChatGPT++" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
echo "$DMG"
