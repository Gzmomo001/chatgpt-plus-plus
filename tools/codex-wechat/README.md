...
它是独立工具，不会修改 ChatGPT++ 管理工具、Codex 配置或桌面注入逻辑。
...
```powershell
uv run tools/codex-wechat/codex_wechat.py login --output tools/codex-wechat/config.local.json
```
...
```powershell
uv run tools/codex-wechat/codex_wechat.py run --config tools/codex-wechat/config.local.json
```
...
- 后续如果要产品化，建议把 thread 映射持久化，并接进 ChatGPT++ 管理工具。
