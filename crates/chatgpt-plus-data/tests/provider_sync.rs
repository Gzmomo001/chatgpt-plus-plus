...
fn provider_sync_target_discovery_reads_all_session_meta_providers() {
    ...
    assert!(ids.contains(&"ChatGPTPlusPlus"));
}

#[test]
fn provider_sync_target_discovery_stops_after_metadata_prefix() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    let rollout = home.join("sessions/2026/rollout-large-body.jsonl");
    ...
    let first = json!({
        ...
        "payload": {
            "id": "thread-1",
            "model_provider": "prefix-only-provider",
            "cwd": "C:/workspace"
        }
    });
    let large_body = json!({
    ...
    let trailing = json!({
        ...
        "payload": {
            "id": "thread-1",
            "model_provider": "body-only-provider",
            "cwd": "C:/workspace"
        }
    });
    ...
    let targets = load_provider_sync_targets(Some(&home));
    let prefix_provider = targets
        .targets
        .iter()
        .find(|target| target.id == "prefix-only-provider")
        .unwrap();

    assert!(
        prefix_provider
            .sources
            .contains(&ProviderSyncTargetSource::Rollout)
    );
    assert!(
        !targets
            .targets
            .iter()
            .any(|target| target.id == "body-only-provider")
    );
}

#[test]
fn provider_sync_target_discovery_keeps_providers_before_split_utf8_boundary() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    let rollout = home.join("sessions/2026/rollout-split-utf8.jsonl");
    fs::create_dir_all(rollout.parent().unwrap()).unwrap();
    fs::write(home.join("config.toml"), "model_provider = \"custom\"\n").unwrap();
    let first = json!({
        "type": "session_meta",
    ...
    let mut rollout_bytes = format!("{first}\n").into_bytes();
    rollout_bytes.resize(64 * 1024 - 1, b'x');
    rollout_bytes.extend_from_slice("中".as_bytes());
...
#[test]
fn provider_sync_target_discovery_reads_exact_limit_line_without_newline() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    let rollout = home.join("sessions/2026/rollout-exact-limit.jsonl");
    fs::create_dir_all(rollout.parent().unwrap()).unwrap();
    fs::write(home.join("config.toml"), "model_provider = \"custom\"\n").unwrap();
    let base = json!({
        "type": "session_meta",
        "payload": {
            "id": "thread-1",
            "model_provider": "exact-limit-provider",
            "cwd": "C:/workspace",
            "padding": ""
        }
    })
    .to_string();
    let padding_len = 64 * 1024 - base.len();
    let exact_limit_line = json!({
        "type": "session_meta",
        "payload": {
            "id": "thread-1",
            "model_provider": "exact-limit-provider",
    ...
    .to_string();
    assert_eq!(exact_limit_line.len(), 64 * 1024);
    fs::write(&rollout, exact_limit_line).unwrap();

    let targets = load_provider_sync_targets(Some(&home));

    assert!(targets.targets.iter().any(|target| {
        target.id == "exact-limit-provider"
            && target.sources.contains(&ProviderSyncTargetSource::Rollout)
    }));
}
...
fn provider_sync_backup_metadata_contains_reference_fields_and_managed_marker() {
    ...
    assert_eq!(metadata["managedBy"], "ChatGPT++ provider sync");
