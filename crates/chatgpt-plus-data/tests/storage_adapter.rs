...
#[test]
fn list_local_sessions_excludes_internal_chatgpt_threads() {
    ...
    .unwrap();
    db.execute(
        "INSERT INTO threads VALUES ('user-thread', '/tmp/user.jsonl', 'Visible chat', 0, 30, 'user')",
        [],
    )
    .unwrap();
    db.execute(
        "INSERT INTO threads VALUES ('guardian-thread', '/tmp/guardian.jsonl', ?1, 0, 20, 'subagent')",
        ["approval transcript".repeat(4_096)],
    )
    .unwrap();
    db.execute(
        "INSERT INTO threads VALUES ('automation-thread', '/tmp/automation.jsonl', 'Scheduled run', 0, 10, 'automation')",
        [],
    )
    .unwrap();
    drop(db);
