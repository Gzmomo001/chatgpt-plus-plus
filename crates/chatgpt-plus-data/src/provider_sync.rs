...
const ROLLOUT_PROVIDER_SCAN_LIMIT_BYTES: u64 = 64 * 1024;
...
pub fn run_provider_sync_with_target(
    ...
    let sync_result = (|| -> anyhow::Result<ProviderSyncResult> {
        ...
        let cwd_by_thread_id = collected
            ...
            .collect::<HashMap<_, _>>();
        let sqlite_paths = chatgpt_plus_core::codex_sqlite::codex_session_db_paths_from_home(&home);
...
pub fn load_provider_sync_targets(codex_home: Option<&Path>) -> ProviderSyncTargetList {
    ...
    for db_path in chatgpt_plus_core::codex_sqlite::codex_session_db_paths_from_home(&home) {
...
fn rollout_provider_ids(home: &Path) -> anyhow::Result<Vec<String>> {
    ...
    for path in rollout_files(home)? {
        ...
        let mut reader = BufReader::new(file.take(ROLLOUT_PROVIDER_SCAN_LIMIT_BYTES));
        ...
        loop {
            ...
            if !line.ends_with(b"\n")
                ...
                && file_len > ROLLOUT_PROVIDER_SCAN_LIMIT_BYTES
            {
                break;
            }
...
fn create_backup(
    ...
    for db_path in chatgpt_plus_core::codex_sqlite::codex_session_db_paths_from_home(home) {
        for source in chatgpt_plus_core::codex_sqlite::codex_sqlite_sidecar_paths(&db_path) {
            if !source.exists() {
                continue;
            }
            let relative = chatgpt_plus_core::codex_sqlite::relative_to_codex_home(home, &source);
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "managedBy": "ChatGPT++ provider sync"
        }))?,
    )?;
    Ok(backup_dir)
}
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if matches!(
            ...
            Some("ChatGPT++ provider sync" | "Codex++ provider sync")
        ) {
            managed.push(path);
        }
    }
