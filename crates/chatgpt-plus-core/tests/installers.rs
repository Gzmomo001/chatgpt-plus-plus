...
fn windows_entrypoint_plan_contains_silent_and_manager_entrypoints() {
    ...
    assert!(plan.silent_shortcut.ends_with("ChatGPT++.lnk"));
    assert!(plan.manager_shortcut.ends_with("ChatGPT++ 管理工具.lnk"));
    assert_eq!(plan.launcher_path, "C:/Tools/chatgpt-plus-plus.exe");
    assert_eq!(plan.manager_path, "C:/Tools/chatgpt-plus-plus-manager.exe");
    assert_eq!(plan.silent_icon_path, "C:/Tools/chatgpt-plus-plus.exe");
    assert_eq!(
...
fn macos_bundle_metadata_contains_silent_and_manager_apps() {
    ...
    assert!(silent.app_path.ends_with("ChatGPT++.app"));
    assert!(manager.app_path.ends_with("ChatGPT++ 管理工具.app"));
    assert!(silent.info_plist.contains("<string>ChatGPT++</string>"));
    assert!(
        manager
            .info_plist
            .contains("<string>ChatGPT++ 管理工具</string>")
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("chatgpt-plus-plus-manager")
    );
    assert!(silent.launch_script.contains("$DIR/chatgpt-plus-plus"));
    assert!(
        manager
            .launch_script
            .contains("$DIR/chatgpt-plus-plus-manager")
    );
}
...
fn companion_binary_path_resolves_macos_manager_app_next_to_silent_app() {
    ...
    let companion =
        companion_binary_path_from_exe(silent_exe, chatgpt_plus_core::install::MANAGER_BINARY);
...
fn macos_bundle_does_not_wrap_the_bundle_executable_in_itself() {
    let options = InstallOptions {
        install_root: Some("/Applications".into()),
        launcher_path: Some("/Applications/ChatGPT++.app/Contents/MacOS/ChatGPTPlusPlus".into()),
        manager_path: Some(
            "/Applications/ChatGPT++ 管理工具.app/Contents/MacOS/ChatGPTPlusPlusManager".into(),
        ),
        remove_owned_data: false,
    };
