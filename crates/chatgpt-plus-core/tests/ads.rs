...
#[test]
fn normalization_does_not_append_local_builtin_sponsors() {
    let payload = normalize_ad_payload(json!({
        "version": 1,
        "ads": [
            {
...
#[test]
fn normalization_does_not_inject_deleted_local_logos() {
    let payload = normalize_ad_payload(json!({
        "version": 1,
        "ads": [
            {
    ...
    for id in [
        ...
        assert!(ad.get("image").is_none(), "{id}");
    }
    let jojocode = ads
        .iter()
...
async fn fetch_ad_list_tries_backup_url_when_primary_fails() {
    ...
    let ads = payload["ads"].as_array().unwrap();
    assert_eq!(ads.len(), 1);
    assert_eq!(ads[0]["id"], json!("backup-ad"));
}
