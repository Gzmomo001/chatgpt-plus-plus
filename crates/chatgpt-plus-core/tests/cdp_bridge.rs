...
use chatgpt_plus_core::bridge::{self, BRIDGE_BINDING_NAME};
use chatgpt_plus_core::cdp::{
...
#[test]
fn runtime_compatibility_embedding_preserves_bridge_and_fallback_contract() {
    let bridge_script = bridge::build_bridge_script(BRIDGE_BINDING_NAME);
    let artifact = assets::injection_script(57321);

    assert_eq!(BRIDGE_BINDING_NAME, "codexSessionDeleteV2");
    assert!(bridge_script.contains("window.__codexSessionDeleteBridge"));
    assert!(bridge_script.contains(BRIDGE_BINDING_NAME));
    assert!(artifact.contains("window.__codexSessionDeleteBridge"));
    assert!(artifact.contains("http://127.0.0.1:57321"));
    assert!(artifact.contains("/backend/status"));
    assert!(artifact.contains("window.__CHATGPT_PLUS_PASTE_FIX__"));
    assert!(artifact.contains("window.__CHATGPT_PLUS_FORCE_CHINESE_LOCALE__"));
    assert!(artifact.contains("window.__CHATGPT_PLUS_FAST_STARTUP__"));
    assert!(artifact.contains("const API_KEY = \"__codexStepwisePanel\""));
    assert!(artifact.contains("root.__chatgptPlusRuntimeCompatibility"));
}

#[test]
fn injection_script_prefixes_helper_url_without_deleted_sponsor_images() {
    let script = assets::injection_script(57321);

    assert!(script.contains("window.__CODEX_SESSION_DELETE_HELPER__"));
    assert!(script.contains("http://127.0.0.1:57321"));
    assert!(!script.contains("window.__CHATGPT_PLUS_SPONSOR_IMAGES__"));
    assert!(!script.contains("请作者喝咖啡"));
    assert!(!script.contains("支付宝赞赏码"));
    assert!(!script.contains("微信赞赏码"));
    assert!(script.contains("window.__CHATGPT_PLUS_VERSION__"));
    assert!(script.contains(chatgpt_plus_core::version::VERSION));
    assert!(script.contains("https://discord.gg/y96kX7A76v"));
    assert!(script.contains("data-chatgpt-plus-discord"));
}
...
fn injection_script_exposes_image_overlay_config() {
    ...
    .unwrap();
    let settings = chatgpt_plus_core::settings::BackendSettings {
    ...
    assert!(script.contains("window.__CHATGPT_PLUS_IMAGE_OVERLAY__"));
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__CHATGPT_PLUS_BUILD__"));
    assert!(script.contains(chatgpt_plus_core::assets::DIAGNOSTIC_BUILD_ID));
    assert!(script.contains("script_loaded"));
    assert!(script.contains("data-chatgpt-plus-build"));
}

#[test]
fn renderer_injection_skips_avatar_overlay_surface() {
    let result = run_surface_injection_harness(
        "app://-/index.html?initialRoute=%2Favatar-overlay",
        "?initialRoute=%2Favatar-overlay",
    );

    assert_eq!(result["surfaceKind"], "auxiliary");
    assert_eq!(result["menuAppended"], false);
    assert_eq!(result["stepwiseLoaded"], false);
}

#[test]
fn renderer_injection_preserves_main_workspace_menu() {
    let result = run_surface_injection_harness("app://-/index.html", "");

    assert_eq!(result["surfaceKind"], "workspace");
    assert_eq!(result["menuAppended"], true);
}

fn run_surface_injection_harness(href: &str, search: &str) -> serde_json::Value {
    let temp = tempfile::tempdir().expect("temp dir should be created");
    let script_path = temp.path().join("renderer-inject.js");
    let harness_path = temp.path().join("avatar-overlay-injection-harness.cjs");
...
function node() {{
  return {{
    appendChild(child) {{ if (child?.id) appendedIds.push(child.id); child.parentElement = this; return child; }},
    prepend(child) {{ child.parentElement = this; return child; }},
...
globalThis.getComputedStyle = () => ({{ gap: "0", columnGap: "0" }});

require(scriptPath);
process.stdout.write(JSON.stringify({{
  appendedIds,
  menuAppended: appendedIds.includes("chatgpt-plus-menu"),
  surfaceKind: window.__chatgptPlusRendererSurface?.kind || null,
  stepwiseLoaded: !!window.__codexStepwisePanel,
}}));
process.exit(0);
"#,
        script_path = serde_json::to_string(&script_path.to_string_lossy().to_string())
            .expect("script path should serialize"),
        href = serde_json::to_string(href).expect("href should serialize"),
        search = serde_json::to_string(search).expect("search should serialize"),
    )
    ...
    let output = Command::new("node")
        .arg(&harness_path)
        .output()
        .expect("node should run avatar overlay injection harness");
    assert!(
    ...
    serde_json::from_slice(&output.stdout).expect("harness should emit JSON")
}
...
fn injection_script_fetches_ads_without_bridge() {
    ...
    assert!(script.contains("directFetchChatGPTPlusAds"));
    assert!(script.contains("cacheBustChatGPTPlusAdUrl"));
    assert!(script.contains("Date.now()"));
    assert!(script.contains("BigPizzaV3/Ad-List"));
    assert!(
        !script.contains("chatgptPlusAds = normalizeChatGPTPlusAds(await postJson(\"/ads\", {}));")
    );
}
...
fn injection_script_menu_exposes_marketplace_plugin_switch_only() {
    ...
    assert!(script.contains("data-chatgpt-plus-setting=\"pluginMarketplaceUnlock\""));
    assert!(!script.contains("特殊插件强制安装"));
    assert!(!script.contains("data-chatgpt-plus-setting=\"forcePluginInstall\""));
    assert!(!script.contains("forcePluginInstall"));
    assert!(!script.contains("强制解锁入口"));
    assert!(!script.contains("data-chatgpt-plus-setting=\"pluginEntryUnlock\""));
}
...
fn injection_script_menu_exposes_stepwise_switch_and_syncs_panel() {
    ...
    assert!(script.contains("data-chatgpt-plus-setting=\"stepwise\""));
...
fn injection_script_defers_backend_mapped_toggles_until_settings_load() {
    ...
    assert!(script.contains("const chatgptPlusBackendMappedSettings = new Set"));
    assert!(script.contains(
        "chatgptPlusBackendMappedSettings.has(key) && !chatgptPlusBackendSettingsLoaded"
    ));
...
fn injection_script_ignores_stale_backend_settings_responses() {
    ...
    assert!(script.contains("let chatgptPlusBackendSettingsSeq = 0"));
    assert!(script.contains("const seq = chatgptPlusBackendSettingsSeq"));
    assert!(script.contains("if (seq !== chatgptPlusBackendSettingsSeq)"));
    assert!(script.contains("const seq = ++chatgptPlusBackendSettingsSeq"));
    assert!(script.contains("if (seq === chatgptPlusBackendSettingsSeq)"));
}
...
fn injection_script_skips_plugin_patch_work_in_relay_mode() {
    ...
    assert!(script.contains("!chatgptPlusBackendSettingsLoaded"));
...
fn injection_script_disables_plugin_auto_expand_in_relay_mode() {
    ...
    assert!(script.contains("if (pluginPatchDisabledInRelayMode()) return"));
    assert!(script.contains("if (!chatgptPlusSettings().pluginAutoExpand) return"));
}
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(
        !script.contains("if (name === \"openai-bundled\") return \"chatgpt-plus-openai-bundled\"")
    );
    assert!(script.contains("if (name === \"openai-bundled\") return \"OpenAI插件1(ChatGPT++)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("__CHATGPT_PLUS_PLUGIN_MARKETPLACES__"));
    ...
    assert!(script.contains("if (name === \"openai-curated\") return \"OpenAI插件2(ChatGPT++)\""));
    assert!(
        script
            .contains("if (name === \"openai-primary-runtime\") return \"OpenAI插件3(ChatGPT++)\"")
    );
    assert!(script.contains("restored === \"openai-api-curated\""));
    assert!(script.contains("restored === \"openai-curated-remote\""));
    assert!(
        script
            .contains("if (name === \"openai-curated-remote\") return \"OpenAI插件5(ChatGPT++)\"")
    );
    assert!(script.contains(
        "if (name === \"chatgpt-plus-openai-curated-remote\") return \"openai-curated-remote\""
    ));
    assert!(script.contains("OpenAI插件1(ChatGPT++)"));
    assert!(script.contains("OpenAI插件2(ChatGPT++)"));
    assert!(script.contains("OpenAI插件3(ChatGPT++)"));
...
fn injection_script_exposes_conversation_view_width_control() {
    ...
    assert!(script.contains("data-chatgpt-plus-conversation-view-width"));
...
fn injection_script_exposes_sidebar_thread_id_badge_control() {
    ...
    assert!(script.contains("data-chatgpt-plus-setting=\"threadIdBadge\""));
...
fn injection_script_exposes_fast_service_tier_control() {
    ...
    assert!(script.contains("codexServiceTierRuntimeRequestHandler"));
    ...
    assert!(script.contains("data-chatgpt-plus-setting=\"serviceTierControls\""));
    ...
    assert!(script.contains("case kinds.StartThread:"));
    assert!(script.contains("case kinds.ResumeThread:"));
    assert!(script.contains("case kinds.StartTurn:"));
...
#[test]
fn service_tier_runtime_registration_fails_closed_when_runtime_is_missing() {
    let result = run_service_tier_runtime_missing_harness();

    assert_eq!(
        result["statuses"],
        json!(["unsupported", "unsupported", "unsupported"])
    );
    assert_eq!(result["runtimeMissingDiagnostics"], 1);
}

fn run_service_tier_runtime_missing_harness() -> serde_json::Value {
    let temp = tempfile::tempdir().expect("temp dir should be created");
    let script_path = temp.path().join("renderer-inject.js");
    let harness_path = temp.path().join("service-tier-runtime-missing-harness.cjs");
    std::fs::write(&script_path, assets::injection_script(57321))
        .expect("injection script should be written");
    let mut harness = std::fs::File::create(&harness_path).expect("harness should be created");
    write!(
...
globalThis.window = globalThis;
window.__CHATGPT_PLUS_TEST_SERVICE_TIER__ = true;
globalThis.document = {{
  scripts: [],
...
globalThis.location = {{ href: "https://codex.test/", pathname: "/", search: "", hash: "" }};
window.location = globalThis.location;
globalThis.navigator = {{ userAgent: "node-test" }};
globalThis.performance = {{ getEntriesByType: () => [] }};

require(scriptPath);
const api = window.__chatgptPlusServiceTierTest;

(async () => {{
  window.ChatGPTPlusRuntimeCompat = null;
  const results = await Promise.all([
    api.installRuntimeInterceptor(),
    api.installRuntimeInterceptor(),
    api.installRuntimeInterceptor(),
  ]);
  const runtimeMissingDiagnostics = api.diagnostics()
    .filter((entry) => entry.event === "runtime_feature_compatibility"
      && entry.detail?.featureId === "service-tier"
      && entry.detail?.status === "unsupported").length;
  process.stdout.write(JSON.stringify({{
    statuses: results.map((result) => result.status),
    runtimeMissingDiagnostics,
  }}));
}})().catch((error) => {{
  console.error(error);
  process.exitCode = 1;
...
"#,
    ...
    let output = Command::new("node")
        .arg(&harness_path)
        .output()
        .expect("node should run service-tier runtime missing harness");
    assert!(
        output.status.success(),
        "node harness failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
...
#[test]
fn injection_script_discovers_app_server_signals_without_hardcoded_hash() {
    let script = assets::injection_script(57321);
    ...
    assert!(!script.contains("import(\"./assets/app-server-manager-signals-"));
}
...
fn injection_script_applies_fast_service_tier_contract() {
    ...
    assert_eq!(cases["adapterContractEqual"], true);
    assert_eq!(cases["worktreeAdapterContractEqual"], true);
    assert_eq!(cases["worktreeNegativeContract"], true);
    assert_eq!(cases["worktreePrepareCalls"], 2);
    assert_eq!(cases["rendererReinjectContract"], true, "{cases}");
    assert_eq!(cases["hotPathFactoryCreateCalls"], 1);
    assert_eq!(cases["runtimeFactoryCreateCalls"], 3);
    assert_eq!(cases["missingRecoveryContract"], true);
    assert_eq!(cases["pluginNoopFinishedDiagnostics"], 0);
    assert_eq!(
        cases["worktreeStartingState"],
        json!({"type": "branch", "branchName": "refs/remotes/upstream/main", "keep": 7})
    );
}
...
fn run_service_tier_contract_harness() -> serde_json::Value {
    let temp = tempfile::tempdir().expect("temp dir should be created");
    let script_path = temp.path().join("renderer-inject.js");
    let renderer_reinject_path = temp.path().join("renderer-reinject.js");
    let harness_path = temp.path().join("service-tier-harness.cjs");
    std::fs::write(&script_path, assets::injection_script(57321))
        .expect("injection script should be written");
    std::fs::write(&renderer_reinject_path, assets::renderer_script())
        .expect("renderer reinjection script should be written");
    let mut harness = std::fs::File::create(&harness_path).expect("harness should be created");
    write!(
        harness,
        r#"
const scriptPath = {script_path};
const rendererReinjectPath = {renderer_reinject_path};
const store = new Map();
store.set("chatgptPlusSettings", JSON.stringify({{ serviceTierControls: true, upstreamWorktreeCreate: true, pluginAutoExpand: true }}));
function node() {{
  return {{
    appendChild() {{}},
    prepend() {{}},
...
globalThis.dispatchEvent = (event) => {{ runtimeEvents.push(event); return event; }};
window.__CHATGPT_PLUS_TEST_SERVICE_TIER__ = true;
window.__CHATGPT_PLUS_TEST_UPSTREAM_WORKTREE__ = true;
globalThis.document = {{
  scripts: [],
  documentElement: node(),
  body: node(),
...
require(scriptPath);
const api = window.__chatgptPlusServiceTierTest;
const worktreeApi = window.__chatgptPlusUpstreamWorktreeTest;
(async () => {{
const pluginDiagnosticsBefore = api.diagnostics().length;
api.schedulePluginAutoExpand();
await new Promise((resolve) => setTimeout(resolve, 250));
api.schedulePluginAutoExpand();
await new Promise((resolve) => setTimeout(resolve, 250));
const pluginNoopFinishedDiagnostics = api.diagnostics().slice(pluginDiagnosticsBefore)
  .filter((entry) => entry.event === "plugin_auto_expand_finished").length;
...
api.setThreadState({{ mode: "global-fast", defaultMode: "fast", entries: {{}} }});
const startConversation = api.handleRuntimeRequest({{
  kind: "StartThread",
  params: {{ threadId: "thread-12345678", model: "gpt-5.5" }},
  threadId: "thread-12345678",
  sourceAdapter: "InlinePayload",
}});

async function throughBundle(raw, handler = api.handleRuntimeRequest) {{
  const calls = [];
  const dispatcher = {{ dispatchMessage(type, payload) {{ calls.push({{ type, payload }}); }} }};
  const runtime = window.ChatGPTPlusRuntimeCompat.createRuntimeCompatibility({{
    root: {{}}, loadModule: async () => ({{ dispatcher }}), maxAttempts: 1, timeoutMs: 100,
  }});
  await runtime.installRequestInterceptor("feature-contract", handler);
  await runtime.installRequestInterceptor("feature-contract", handler);
  const {{ type, ...payload }} = raw;
  dispatcher.dispatchMessage(type, payload);
  return calls.at(-1);
}}
async function throughWindowEvent(raw, handler = api.handleRuntimeRequest) {{
  const calls = [];
  const root = {{ dispatchEvent(event) {{ calls.push(event); return true; }} }};
  const runtime = window.ChatGPTPlusRuntimeCompat.createRuntimeCompatibility({{
    root, createEvent: (type, detail) => ({{ type, detail }}), maxAttempts: 1, timeoutMs: 100,
  }});
  await runtime.installRequestInterceptor("feature-contract", handler);
  await runtime.installRequestInterceptor("feature-contract", handler);
  root.dispatchEvent({{ type: "codex-message-from-view", detail: raw }});
  const detail = calls.at(-1).detail;
  const {{ type, ...payload }} = detail;
  return {{ type, payload }};
}}
const adapterCases = [
  {{ type: "mcp-request", request: {{ method: "thread/start", params: {{ model: "gpt-5.5" }} }} }},
  {{ type: "send-cli-request-for-host", method: "thread/resume", params: {{ threadId: "thread-12345678", model: "gpt-5.5" }} }},
  {{ type: "start-turn-for-host", conversationId: "thread-12345678", params: {{ model: "gpt-5.5", input: [] }} }},
];
let adapterContractEqual = true;
for (const raw of adapterCases) {{
  const bundle = await throughBundle(raw);
  const event = await throughWindowEvent(raw);
  adapterContractEqual &&= JSON.stringify(bundle) === JSON.stringify(event);
}}
let worktreePrepareCalls = 0;
worktreeApi.setPrepareObserver(() => {{ worktreePrepareCalls += 1; }});
worktreeApi.setSelection({{ repoPath: "/repo", projectId: "project-1", remote: "upstream", baseBranch: "main", qualifiedSourceRef: "refs/remotes/upstream/main", label: "upstream/main" }});
const worktreeRaw = {{ type: "pending-worktree-create", request: {{
  launchMode: "start-conversation", sourceWorkspaceRoot: "/repo", untouched: {{ nested: true }},
  startingState: {{ type: "branch", branchName: "refs/heads/main", keep: 7 }},
}} }};
const bundleWorktree = await throughBundle(worktreeRaw, worktreeApi.handleRuntimeRequest);
const eventWorktree = await throughWindowEvent(worktreeRaw, worktreeApi.handleRuntimeRequest);
const worktreeAdapterContractEqual = JSON.stringify(bundleWorktree) === JSON.stringify(eventWorktree)
  && worktreeRaw.request.startingState.branchName === "refs/heads/main"
  && bundleWorktree.payload.request.untouched.nested === true;
const worktreeStartingState = bundleWorktree.payload.request.startingState;
let negativeHandlerCalls = 0;
const gatedWorktreeHandler = (request) => {{
  negativeHandlerCalls += 1;
  return worktreeApi.handleRuntimeRequest(request);
}};
async function rejectedThroughBundle(raw) {{
  const calls = [];
  const dispatcher = {{ dispatchMessage(type, payload) {{ calls.push({{ type, payload }}); }} }};
  const runtime = window.ChatGPTPlusRuntimeCompat.createRuntimeCompatibility({{
    root: {{}}, loadModule: async () => ({{ dispatcher }}), maxAttempts: 1, timeoutMs: 100,
  }});
  await runtime.installRequestInterceptor("worktree-negative", gatedWorktreeHandler);
  const {{ type, ...payload }} = raw;
  dispatcher.dispatchMessage(type, payload);
  return calls.at(-1).payload === payload && payload.request.startingState.branchName === "refs/heads/main";
}}
async function rejectedThroughWindowEvent(raw) {{
  const calls = [];
  const root = {{ dispatchEvent(event) {{ calls.push(event); return true; }} }};
  const runtime = window.ChatGPTPlusRuntimeCompat.createRuntimeCompatibility({{
    root, createEvent: (type, detail) => ({{ type, detail }}), maxAttempts: 1, timeoutMs: 100,
  ...
  await runtime.installRequestInterceptor("worktree-negative", gatedWorktreeHandler);
  const event = {{ type: "codex-message-from-view", detail: raw }};
  root.dispatchEvent(event);
  return calls.at(-1) === event && raw.request.startingState.branchName === "refs/heads/main";
}}
let worktreeNegativeContract = true;
for (const launchMode of ["open-existing", undefined]) {{
  const request = {{ sourceWorkspaceRoot: "/repo", startingState: {{ type: "branch", branchName: "refs/heads/main" }} }};
  if (launchMode !== undefined) request.launchMode = launchMode;
  const raw = {{ type: "pending-worktree-create", request }};
  worktreeNegativeContract &&= await rejectedThroughBundle(raw);
  worktreeNegativeContract &&= await rejectedThroughWindowEvent(raw);
}}
worktreeNegativeContract &&= negativeHandlerCalls === 0;

const nativeRuntimeFactory = window.ChatGPTPlusRuntimeCompat;
let runtimeFactoryCreateCalls = 0;
const countingRuntimeFactory = {{
  apiVersion: nativeRuntimeFactory.apiVersion,
  createRuntimeCompatibility(environment) {{
    runtimeFactoryCreateCalls += 1;
    return nativeRuntimeFactory.createRuntimeCompatibility(environment);
  }},
}};
window.ChatGPTPlusRuntimeCompat = countingRuntimeFactory;
api.setModelCatalog({{ status: "ok", model: "gpt-5.5", default_model: "gpt-5.5", models: ["gpt-5.5"] }});
api.setThreadState({{ mode: "global-fast", defaultMode: "fast", entries: {{}} }});
api.setServiceTierState({{ serviceTier: "priority", fastTierValue: "priority" }});
const firstInstallPromise = api.installRuntimeInterceptor();
const duplicateInstallPromise = api.installRuntimeInterceptor();
await Promise.all([firstInstallPromise, duplicateInstallPromise]);
const transportAfterFirstInstall = window.dispatchEvent;
const dispatchRuntimeRequest = () => {{
  window.dispatchEvent({{ type: "codex-message-from-view", detail: {{
    type: "mcp-request", request: {{ method: "thread/start", params: {{}} }},
  }} }});
  return runtimeEvents.at(-1).detail.request.params.serviceTier;
}};
dispatchRuntimeRequest();
const oldHandlerServiceTier = dispatchRuntimeRequest();
const hotPathFactoryCreateCalls = runtimeFactoryCreateCalls;

delete require.cache[require.resolve(rendererReinjectPath)];
require(rendererReinjectPath);
const reinjectedApi = window.__chatgptPlusServiceTierTest;
reinjectedApi.setModelCatalog({{ status: "ok", model: "gpt-4.1", default_model: "gpt-4.1", models: ["gpt-4.1"] }});
reinjectedApi.setThreadState({{ mode: "global-fast", defaultMode: "fast", entries: {{}} }});
reinjectedApi.setServiceTierState({{ serviceTier: "priority", fastTierValue: "priority" }});
await reinjectedApi.installRuntimeInterceptor();
const transportAfterReinject = window.dispatchEvent;
const newHandlerServiceTier = dispatchRuntimeRequest();
const rendererReinjectContract = transportAfterFirstInstall === transportAfterReinject
  && oldHandlerServiceTier === "priority"
  && newHandlerServiceTier === null
  && firstInstallPromise === duplicateInstallPromise
  && hotPathFactoryCreateCalls === 1
  && runtimeFactoryCreateCalls === 2;

window.ChatGPTPlusRuntimeCompat = null;
delete require.cache[require.resolve(rendererReinjectPath)];
require(rendererReinjectPath);
const recoveryApi = window.__chatgptPlusServiceTierTest;
const diagnosticsBeforeRecovery = recoveryApi.diagnostics().length;
const missingInstallPromise = recoveryApi.installRuntimeInterceptor();
const duplicateMissingInstallPromise = recoveryApi.installRuntimeInterceptor();
const missingRuntimeResult = await missingInstallPromise;
window.ChatGPTPlusRuntimeCompat = countingRuntimeFactory;
const recoveredRuntimeResult = await recoveryApi.installRuntimeInterceptor();
const recoveryDiagnostics = recoveryApi.diagnostics().slice(diagnosticsBeforeRecovery)
  .filter((entry) => entry.event === "runtime_feature_compatibility" && entry.detail?.featureId === "service-tier")
  .map((entry) => entry.detail.status);
const missingRecoveryContract = missingRuntimeResult.status === "unsupported"
  && recoveredRuntimeResult.status === "supported"
  && missingInstallPromise === duplicateMissingInstallPromise
  && recoveryDiagnostics.includes("unsupported")
  && recoveryDiagnostics.includes("supported")
  && window.dispatchEvent === transportAfterFirstInstall
  && runtimeFactoryCreateCalls === 3;
...
process.stdout.write(JSON.stringify({{
  ...
  startConversation,
  adapterContractEqual,
  worktreeAdapterContractEqual,
  worktreeNegativeContract,
  worktreePrepareCalls,
  worktreeStartingState,
  rendererReinjectContract,
  hotPathFactoryCreateCalls,
  runtimeFactoryCreateCalls,
  missingRecoveryContract,
  pluginNoopFinishedDiagnostics,
  oldHandlerServiceTier,
  newHandlerServiceTier,
  transportWrapperStable: transportAfterFirstInstall === transportAfterReinject,
  missingRuntimeStatus: missingRuntimeResult.status,
  recoveredRuntimeStatus: recoveredRuntimeResult.status,
  recoveryDiagnostics,
}}));
...
"#,
        script_path = serde_json::to_string(&script_path.to_string_lossy().to_string())
            .expect("script path should serialize"),
        renderer_reinject_path = serde_json::to_string(
            &renderer_reinject_path.to_string_lossy().to_string()
        )
        .expect("renderer reinjection path should serialize")
    )
    .expect("harness should be written");
    drop(harness);
...
fn injection_script_installs_upstream_branch_dropdown_adapter() {
    ...
    assert!(script.contains("upstreamWorktreeRuntimeRequestHandler"));
...
#[test]
fn codex_target_selection_prefers_main_window_over_avatar_overlay() {
    let targets = vec![
        target(
            "avatar-overlay",
            "page",
            "Codex",
            "app://-/index.html?initialRoute=%2Favatar-overlay",
            Some("ws://127.0.0.1/avatar-overlay"),
        ),
        target(
            "main-window",
            "page",
            "Codex",
            "app://-/index.html",
            Some("ws://127.0.0.1/main-window"),
        ),
    ];

    assert_eq!(
        pick_injectable_codex_page_target(&targets)
            .expect("main Codex target should be selected")
            .id,
        "main-window"
    );
    assert_eq!(
        pick_page_target(&targets)
            .expect("main page target should be selected")
            .id,
        "main-window"
    );
}
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    let temp = tempfile::tempdir().unwrap();
    let log_path = temp.path().join("chatgpt-plus.log");
    chatgpt_plus_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    ...
    chatgpt_plus_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
