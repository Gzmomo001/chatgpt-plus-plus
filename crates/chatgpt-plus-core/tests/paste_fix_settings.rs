use chatgpt_plus_core::assets::paste_fix_enabled_config;
...
fn injection_script_includes_paste_fix_global() {
    use chatgpt_plus_core::assets::injection_script_with_settings;

    let mut settings = BackendSettings::default();
    settings.codex_app_paste_fix = true;
    let script = injection_script_with_settings(0, &settings);
    assert!(
        script.contains("window.__CHATGPT_PLUS_PASTE_FIX__ = {\"enabled\":true};"),
        "script should declare __CHATGPT_PLUS_PASTE_FIX__ as enabled, got: {}",
        &script[..script
            .find("window.__CHATGPT_PLUS_PASTE_FIX__")
            .unwrap_or(0)
            + 80]
    );
    ...
    let script = injection_script_with_settings(0, &settings);
    assert!(
        script.contains("window.__CHATGPT_PLUS_PASTE_FIX__ = {\"enabled\":false};"),
        "script should declare __CHATGPT_PLUS_PASTE_FIX__ as disabled"
    );
}
