...
use chatgpt_plus_core::app_paths::{
...
use chatgpt_plus_core::ports::{
    select_packaged_codex_debug_port_with, select_platform_loopback_port_with,
};
use chatgpt_plus_core::settings::{BackendSettings, RelayProfile, RelayProtocol};
...
fn launcher_hook_surface_does_not_expose_relay_profile_application() {
    ...
    assert!(!launcher_source.contains(&removed_hook));
}
...
#[test]
fn launcher_applies_chatgptplusplus_window_icon_after_packaged_activation() {
    let source = include_str!("../src/launcher.rs");

    assert!(source.contains("apply_chatgptplusplus_window_icon_after_launch(process_id);"));
    assert!(source.contains("windows_apply_chatgptplusplus_icon_to_process_window"));
}
...
fn launcher_no_longer_contains_mobile_control_runtime() {
    ...
    assert!(!workspace_toml.contains("apps/chatgpt-plus-mobile-relay"));
    assert!(!lockfile.contains("name = \"chatgpt-plus-mobile-relay\""));
    ...
    assert!(!launcher_source.contains("CHATGPT_PLUS_MOBILE"));
    assert!(!settings_source.contains("mobileControl"));
}

#[test]
fn repository_no_longer_contains_mobile_relay_artifacts() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let workspace_root = manifest_dir
        .parent()
        .and_then(Path::parent)
        .expect("core crate should be nested under the workspace root");

    assert!(
        !workspace_root
            .join("apps/chatgpt-plus-mobile-relay")
            .exists()
    );
}
...
async fn launch_lifecycle_runs_enabled_maintenance_without_applying_relay_profile() {
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_launch_result(CodexLaunch::Process {
            ...
            wait_strategy: chatgpt_plus_core::launcher::ProcessWaitStrategy::TrackedChild,
...
async fn launch_starts_helper_when_chat_protocol_proxy_is_enabled() {
    ...
    let settings = BackendSettings {
        ...
        relay_profiles: vec![RelayProfile {
            ...
            relay_mode: chatgpt_plus_core::settings::RelayMode::MixedApi,
            official_mix_api_key: false,
            test_model: String::new(),
            config_contents: String::new(),
            auth_contents: String::new(),
            use_common_config: true,
            context_selection: chatgpt_plus_core::settings::RelayContextSelection::default(),
            context_selection_initialized: false,
            context_window: String::new(),
            auto_compact_limit: String::new(),
            model_insert_mode: chatgpt_plus_core::settings::RelayModelInsertMode::default(),
