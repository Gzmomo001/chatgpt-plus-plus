use chatgpt_plus_core::assets::{force_chinese_locale_config, injection_script_with_settings};
...
fn injection_script_includes_force_chinese_locale_global_and_patch() {
    ...
    assert!(script.contains(
        "window.__CHATGPT_PLUS_FORCE_CHINESE_LOCALE__ = {\"enabled\":true,\"locale\":\"zh-CN\"};"
    ));
    assert!(script.contains(
        "window.__CHATGPT_PLUS_FAST_STARTUP__ = {\"enabled\":true,\"statsigTimeoutMs\":800};"
    ));
    assert!(script.contains("__chatgptPlusForceChineseLocaleInstalled"));
    assert!(script.contains("__chatgptPlusFastStartupInstalled"));
    ...
    let script = injection_script_with_settings(0, &settings);
    assert!(script.contains(
        "window.__CHATGPT_PLUS_FORCE_CHINESE_LOCALE__ = {\"enabled\":false,\"locale\":\"zh-CN\"};"
    ));
}
