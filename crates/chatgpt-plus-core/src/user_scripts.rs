...
fn wrap_script(script: &UserScriptFile, source: &str) -> String {
    format!(
        r#"
(() => {{
  window.__chatgptPlusUserScripts = window.__chatgptPlusUserScripts || {{ scripts: {{}} }};
  const key = {key};
  window.__chatgptPlusUserScripts.scripts[key] = {{ key, name: {name}, source: {source_name}, status: "loading", error: "", loadedAt: new Date().toISOString() }};
  try {{
{source}
    window.__chatgptPlusUserScripts.scripts[key].status = "loaded";
    window.__chatgptPlusUserScripts.scripts[key].loadedAt = new Date().toISOString();
  }} catch (error) {{
    window.__chatgptPlusUserScripts.scripts[key].status = "failed";
    window.__chatgptPlusUserScripts.scripts[key].error = String(error && (error.stack || error.message) || error);
  }}
}})();
"#,
