use super::{
    ChatSseToResponsesConverter, ProtocolProxyRequest, chat_completion_to_response,
    chat_completion_to_response_with_request, chat_completions_url, chat_sse_to_responses_sse,
    chat_sse_to_responses_sse_with_request, models_url, open_chat_completions_proxy_request,
    open_models_proxy_request, open_responses_proxy_request,
    open_responses_proxy_request_with_settings, protocol_proxy_transaction,
...
#[tokio::test]
async fn transaction_handles_models_options_without_opening_an_upstream() {
    let mut response = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "OPTIONS",
        "/v1/models?limit=10",
        Vec::new(),
        Some("Codex/26.614".to_string()),
    ))
    .await
    .unwrap()
    .expect("models alias should be handled");
...
#[tokio::test]
async fn transaction_recognizes_every_supported_route_alias() {
    for path in [
        "/responses",
        "/v1/responses",
        "/v1/v1/responses",
        ...
        let result =
            protocol_proxy_transaction(ProtocolProxyRequest::new("POST", path, vec![0xff], None))
                .await;
        assert!(
            result.is_err(),
            "{path} must be handled before invalid UTF-8 reaches an upstream"
        );
    }

    for path in ["/models", "/v1/models", "/v1/v1/models", "/codex/v1/models"] {
        let response = protocol_proxy_transaction(ProtocolProxyRequest::new(
            "OPTIONS",
            path,
            Vec::new(),
            None,
        ...
        .await
        .unwrap();
        assert_eq!(response.unwrap().status(), "204 No Content", "{path}");
    }
}

#[tokio::test(start_paused = true)]
async fn transaction_applies_production_header_timeout_to_hung_upstream() {
    ...
    write_responses_relay_settings(temp.path(), &format!("http://{address}/v1"), "");

    let started = tokio::time::Instant::now();
    let error = match protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/v1/responses",
        br#"{"model":"gpt-5-mini","input":"hello"}"#.to_vec(),
        None,
    ))
    .await
    {
        Err(error) => error,
        Ok(_) => panic!("hung upstream must time out"),
    };
...
#[tokio::test(start_paused = true)]
async fn direct_chat_transaction_applies_production_header_timeout() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    write_chat_relay_settings(temp.path(), &format!("http://{address}/v1"), "");

    let started = tokio::time::Instant::now();
    let result = tokio::time::timeout(
        Duration::from_secs(31),
        protocol_proxy_transaction(ProtocolProxyRequest::new(
            "POST",
            "/v1/chat/completions",
            br#"{"model":"gpt-5-mini","messages":[]}"#.to_vec(),
            None,
        )),
    )
    .await;
    let error = match result {
        ...
        Ok(Ok(_)) => panic!("hung direct Chat upstream must not succeed"),
        Err(_) => panic!("transaction did not apply its production header timeout"),
    };
...
#[tokio::test]
async fn direct_chat_stream_request_buffers_json_upstream_error() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let mut response = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/chat/completions",
        br#"{"model":"gpt-5-mini","messages":[],"stream":true}"#.to_vec(),
        None,
    ))
    .await
    .unwrap()
...
#[tokio::test]
async fn transaction_passes_responses_upstream_status_content_type_and_body_through() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    .await
    .unwrap()
    .expect("responses route should be handled");

    assert_eq!(response.status(), "201 Upstream");
    assert_eq!(response.content_type(), "application/x-test+json");
    assert_eq!(
        response.next_chunk().await.unwrap().unwrap(),
        br#"{"id":"resp_tx","object":"response"}"#
    );
    assert_eq!(response.next_chunk().await.unwrap(), None);
    server.await.unwrap();
}
...
#[tokio::test]
async fn transaction_converts_responses_to_chat_and_chat_back_to_responses() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let server = tokio::spawn(async move {
        ...
        let body = r#"{"id":"chatcmpl_tx","created":7,"model":"gpt-5-mini","choices":[{"finish_reason":"stop","message":{"role":"assistant","content":"hello back"}}],"usage":{"prompt_tokens":3,"completion_tokens":2,"total_tokens":5}}"#;
        let response = format!(
            "HTTP/1.1 200 OK\r\ncontent-length: {}\r\ncontent-type: application/json\r\n\r\n{}",
            body.len(),
            body
    ...
    let mut response = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/codex/v1/responses",
        br#"{"model":"gpt-5-mini","instructions":"be concise","input":"hello"}"#.to_vec(),
        None,
    ))
    .await
    .unwrap()
    .expect("responses alias should be handled for chat upstream");
    let body: serde_json::Value =
        serde_json::from_slice(&response.next_chunk().await.unwrap().unwrap()).unwrap();
    let upstream_request = server.await.unwrap();
    let upstream_body: serde_json::Value = serde_json::from_str(
        upstream_request
            .split_once("\r\n\r\n")
            .expect("upstream request should contain a body")
            .1,
    )
    .unwrap();
...
#[tokio::test]
async fn transaction_converts_chat_sse_across_partial_utf8_network_chunks() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let server = tokio::spawn(async move {
        ...
        stream
            ...
            .unwrap();
        let sse = concat!(
            "data: {\"id\":\"chatcmpl_sse\",\"created\":1,\"model\":\"gpt-5-mini\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\"}}]}\n\n",
            "data: {\"id\":\"chatcmpl_sse\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"你\"}}]}\n\n",
            "data: {\"id\":\"chatcmpl_sse\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n",
            "data: [DONE]\n\n"
        );
        let marker = sse
            .as_bytes()
            ...
            .unwrap();
        for chunk in [&sse.as_bytes()[..marker + 1], &sse.as_bytes()[marker + 1..]] {
            stream
                .write_all(format!("{:x}\r\n", chunk.len()).as_bytes())
                .await
                .unwrap();
    ...
    let mut response = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/responses",
        br#"{"model":"gpt-5-mini","input":"hello","stream":true}"#.to_vec(),
        None,
    ))
    .await
    .unwrap()
    .expect("streaming responses route should be handled");
    ...
    assert!(output.contains("\\u4f60") || output.contains('你'));
    assert!(output.contains("event: response.completed"));
}

#[tokio::test]
async fn transaction_turns_chat_upstream_midstream_failure_into_responses_sse_failure() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let server = tokio::spawn(async move {
        ...
        stream
            ...
            .await
            .unwrap();
        let first = b"data: {\"id\":\"chatcmpl_fail\",\"created\":1,\"model\":\"gpt-5-mini\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"before failure\"}}]}\n\n";
        stream
            .write_all(format!("{:x}\r\n", first.len()).as_bytes())
            .await
            .unwrap();
...
#[tokio::test]
async fn transaction_marks_native_passthrough_stream_transport_failure() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
...
#[tokio::test]
async fn transaction_does_not_complete_truncated_non_stream_body() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    assert!(
        saw_transport_error,
        "truncated JSON must not become a complete body"
    );
    assert!(!response.is_success());
}

#[tokio::test]
async fn transaction_prefers_configured_user_agent_and_otherwise_preserves_original() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));

    let (configured_url, configured_request) = spawn_responses_capture_server().await;
    write_responses_relay_settings(temp.path(), &configured_url, "Configured-UA/1.0");
    let mut configured = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/responses",
        br#"{"model":"gpt-5-mini","input":"hello"}"#.to_vec(),
        Some("Original-UA/1.0".to_string()),
    ))
    .await
    .unwrap()
    .unwrap();
    while configured.next_chunk().await.unwrap().is_some() {}
    assert!(
        configured_request
            .await
            .unwrap()
            .contains("user-agent: configured-ua/1.0")
    );

    let (original_url, original_request) = spawn_responses_capture_server().await;
    write_responses_relay_settings(temp.path(), &original_url, "");
    let mut original = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/responses",
        br#"{"model":"gpt-5-mini","input":"hello"}"#.to_vec(),
        Some("Original-UA/1.0".to_string()),
    ...
    .unwrap()
    .unwrap();
    while original.next_chunk().await.unwrap().is_some() {}
    assert!(
        original_request
            .await
            .unwrap()
            .contains("user-agent: original-ua/1.0")
    );
}

#[tokio::test]
async fn transaction_regularizes_chat_upstream_error_status_and_content_type() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    .await
    .unwrap()
    .expect("upstream errors remain handled responses transactions");
    let body: serde_json::Value =
        serde_json::from_slice(&response.next_chunk().await.unwrap().unwrap()).unwrap();
...
#[tokio::test]
async fn transaction_forwards_models_alias_with_upstream_status_and_content_type() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let server = tokio::spawn(respond_once(
        listener,
        "HTTP/1.1 206 Partial Content\r\ncontent-type: application/models+json\r\ncontent-length: 11\r\n\r\n{\"data\":[]}",
    ));
    write_responses_relay_settings(temp.path(), &format!("http://{address}/v1"), "");

    let mut response = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "GET",
        "/codex/v1/models?limit=10",
        Vec::new(),
        Some("Codex/26.614".to_string()),
    ))
    .await
    ...
    assert_eq!(response.status(), "206 Upstream");
    assert_eq!(response.content_type(), "application/models+json");
    assert_eq!(
        response.next_chunk().await.unwrap().unwrap(),
        br#"{"data":[]}"#
    );
...
#[tokio::test]
async fn transaction_passes_chat_completions_route_through_without_conversion() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    let server = tokio::spawn(respond_once(
        listener,
        "HTTP/1.1 202 Accepted\r\ncontent-type: application/chat+json\r\ncontent-length: 20\r\n\r\n{\"id\":\"chat_direct\"}",
    ));
    write_chat_relay_settings(temp.path(), &format!("http://{address}/v1"), "");
    ...
    .await
    .unwrap()
    .expect("chat completions alias should be handled");

    assert_eq!(response.status(), "202 Upstream");
    assert_eq!(response.content_type(), "application/chat+json");
    assert_eq!(
        response.next_chunk().await.unwrap().unwrap(),
        br#"{"id":"chat_direct"}"#
    );
...
async fn aggregate_proxy_fails_over_to_next_member_in_same_request() {
    ...
    let mut stored_settings = serde_json::to_value(&settings).unwrap();
    for (stored, profile) in stored_settings["relayProfiles"]
        .as_array_mut()
        .unwrap()
        .iter_mut()
        .zip(&settings.relay_profiles)
    {
        stored["apiKey"] = json!(profile.api_key);
    }
    std::fs::write(
        temp.path().join("settings.json"),
        serde_json::to_vec_pretty(&stored_settings).unwrap(),
    )
    .unwrap();
    let loaded = SettingsStore::default().load().unwrap();
    assert!(
        loaded.relay_profiles[0].base_url.starts_with("http://"),
        "stored aggregate member lost base URL: {:?}",
        loaded.relay_profiles[0]
    );
    assert_eq!(loaded.relay_profiles[0].api_key, "sk-first");
    let original_body = br#"{"model":"gpt-5-mini","input":"hi","stream":false}"#.to_vec();
    let mut result = protocol_proxy_transaction(ProtocolProxyRequest::new(
        "POST",
        "/v1/responses",
        original_body.clone(),
        None,
    ...
    .unwrap()
    .unwrap();
    let body = result.next_chunk().await.unwrap().unwrap();
    ...
    assert_eq!(
        ...
        "failover must reuse the request body"
    );
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(second_body).unwrap(),
        serde_json::from_slice::<serde_json::Value>(&original_body).unwrap()
    );
}
...
fn aggregate_proxy_settings(
    ...
    let first_config = format!(
        "model_provider = \"chatgpt-plus-relay\"\n[model_providers.chatgpt-plus-relay]\nbase_url = \"{first_base_url}\"\n"
    );
    ...
    BackendSettings {
        relay_profiles: vec![
            RelayProfile {
                ...
                upstream_base_url: first_base_url,
                api_key: "sk-first".to_string(),
                relay_mode: RelayMode::MixedApi,
                config_contents: first_config,
                auth_contents: r#"{"OPENAI_API_KEY":"sk-first"}"#.to_string(),
            ...
            RelayProfile {
                ...
                upstream_base_url: second_base_url,
                api_key: "sk-second".to_string(),
                relay_mode: RelayMode::MixedApi,
                config_contents: second_config,
                auth_contents: r#"{"OPENAI_API_KEY":"sk-second"}"#.to_string(),
                ..RelayProfile::default()
            },
            RelayProfile {
...
fn write_responses_relay_settings(settings_dir: &Path, base_url: &str, user_agent: &str) {
    let settings = json!({
        "relayProfiles": [{
            "id": "responses",
            "name": "Responses",
...
async fn spawn_responses_capture_server() -> (String, tokio::task::JoinHandle<String>) {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
        .await
        .unwrap();
    let address = listener.local_addr().unwrap();
    let handle = tokio::spawn(async move {
        ...
        let read = stream.read(&mut buffer).await.unwrap();
        let request = String::from_utf8_lossy(&buffer[..read]).to_string();
        let body = r#"{"id":"resp_ua","object":"response"}"#;
        stream
            .write_all(
                format!(
                    "HTTP/1.1 200 OK\r\ncontent-type: application/json\r\ncontent-length: {}\r\n\r\n{}",
...
impl SettingsPathGuard {
    fn set(path: PathBuf) -> Self {
        let previous = chatgpt_plus_core::paths::set_settings_path_for_tests(Some(path));
        Self { previous }
    }
}
...
impl Drop for SettingsPathGuard {
    fn drop(&mut self) {
        chatgpt_plus_core::paths::set_settings_path_for_tests(self.previous.take());
    }
}
