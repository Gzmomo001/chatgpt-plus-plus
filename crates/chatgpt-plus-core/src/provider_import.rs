...
pub fn request_from_url(url: &str) -> anyhow::Result<ProviderImportRequest> {
    let (path, query) = url.split_once('?').context("导入链接缺少查询参数")?;
    let scheme = path
        .split_once("://")
        .map(|(scheme, _)| scheme)
        .context("导入链接缺少协议")?;
    if !matches!(scheme, "chatgptplusplus" | "codexplusplus") {
        anyhow::bail!("不支持的供应商导入协议：{scheme}");
    }
...
fn build_config_toml(base_url: &str, api_key: &str, protocol: RelayProtocol) -> String {
    ...
    [
        "model_provider = \"ChatGPTPlusPlus\"".to_string(),
        String::new(),
        "[model_providers.ChatGPTPlusPlus]".to_string(),
        "name = \"ChatGPTPlusPlus\"".to_string(),
...
mod tests {
    ...
    #[test]
    fn parses_legacy_codexplusplus_provider_url() {
    ...
    #[test]
    fn parses_chatgptplusplus_provider_url() {
        let url = "chatgptplusplus://v1/import/provider?resource=provider&name=JOJO%20Code&baseUrl=https%3A%2F%2Fjojocode.com%2Fv1&apiKey=sk-test";
    ...
    #[test]
    fn rejects_unrelated_provider_url_scheme() {
        let url = "https://example.com/v1/import/provider?resource=provider&name=Example&baseUrl=https%3A%2F%2Fexample.com%2Fv1&apiKey=sk-test";

        let error = request_from_url(url).unwrap_err();
