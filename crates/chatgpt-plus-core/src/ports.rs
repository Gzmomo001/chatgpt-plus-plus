...
pub fn manager_guard_port() -> u16 {
    if let Some(port) =
        crate::branding::env_var_with_legacy("CHATGPT_PLUS_GUARD_PORT", "CODEX_PLUS_GUARD_PORT")
            .or_else(|_| {
                crate::branding::env_var_with_legacy(
                    "CHATGPT_PLUS_MANAGER_GUARD_PORT",
                    "CODEX_PLUS_MANAGER_GUARD_PORT",
                )
            })
            .ok()
            .and_then(|v| v.parse::<u16>().ok())
...
mod tests {
    ...
    fn manager_guard_port_honors_specific_env_override() {
        let _guard = guard_port_env_lock();
        _clear_guard_port_env_vars();
        unsafe { std::env::set_var("CHATGPT_PLUS_MANAGER_GUARD_PORT", "7777") };
        let port = manager_guard_port();
        unsafe { std::env::remove_var("CHATGPT_PLUS_MANAGER_GUARD_PORT") };
        assert_eq!(port, 7777);
    }
