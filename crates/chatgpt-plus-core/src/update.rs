...
fn is_macos_native_arch_asset(name: &str) -> bool {
    ...
    // Underscore filename shape: `ChatGPTPlusPlus_1.0.9_x64.dmg`.
...
fn is_macos_installer_asset(name: &str) -> bool {
    ...
    let lower = name.to_ascii_lowercase();
    branded_asset_name(&lower) && lower.ends_with(".dmg")
}

fn branded_asset_name(lower: &str) -> bool {
    (lower.contains("chatgpt") || lower.contains("codex")) && lower.contains("plus")
}
