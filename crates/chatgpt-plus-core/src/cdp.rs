...
pub fn pick_page_target(targets: &[CdpTarget]) -> anyhow::Result<CdpTarget> {
    ...
    let mut preferred_codex = None;
    let mut preferred_rank = 0;
    for target in targets
        .iter()
        .filter(|target| is_injectable_page_target(target))
    {
        first_page.get_or_insert(target);
        let rank = codex_page_target_rank(target);
        if rank > preferred_rank {
            preferred_codex = Some(target);
            preferred_rank = rank;
        }
    }

    if let Some(target) = preferred_codex {
...
fn codex_page_target_rank(target: &CdpTarget) -> u8 {
    ...
    if url.contains("initialroute=%2favatar-overlay")
